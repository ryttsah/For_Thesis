...
interface VisitOutcomeModalProps {
  ...
  farm: string;
  onClose: () => void;
  onSave: (value: { visited: boolean; officerComment: string; notVisitedReason: string }) => Promise<void>;
}
...
export default function VisitOutcomeModal({ open, farm, onClose, onSave }: VisitOutcomeModalProps) {
  ...
  async function submit() {
    ...
    try { await onSave({ visited, officerComment, notVisitedReason }); onClose(); } catch (e) { setError(e instanceof Error ? e.message : "Could not save the visit log."); }
