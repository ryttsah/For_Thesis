...
interface VisitOutcomeModalProps {
  ...
  farm: string;
  onClose: () => void;
  onSave: (value: { visited: boolean; officerComment: string; notVisitedReason: string; evidenceImage: string }) => Promise<void>;
}
...
export default function VisitOutcomeModal({ open, farm, onClose, onSave }: VisitOutcomeModalProps) {
  ...
  async function submit() {
    ...
    if (visited && (!evidence || !evidencePreview)) { setError("Take a fresh farm-visit photo before saving this outcome."); return; }
    setSaving(true); setError("");
    try { await onSave({ visited, officerComment, notVisitedReason, evidenceImage: evidencePreview }); onClose(); } catch (e) { setError(e instanceof Error ? e.message : "Could not save the visit log."); }
    finally { setSaving(false); }
  }

  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4" role="dialog" aria-modal="true">
      <div className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:p-6">
        ...
        {visited ? <><label className="mt-4 block text-sm font-semibold">Work completed<textarea value={officerComment} onChange={(e) => setOfficerComment(e.target.value)} className="mt-2 min-h-28 w-full rounded-lg border border-pca-border p-3 text-sm font-normal" placeholder="Describe the inspection, advice, treatment, or survey work done." /></label><label className="mt-4 flex cursor-pointer items-center justify-between rounded-xl border border-pca-green bg-pca-green-light p-4 text-sm font-bold text-pca-green"><span className="flex items-center gap-2"><IconCamera size={20} />Take fresh farm-visit evidence</span><span>{evidencePreview ? <span className="inline-flex items-center gap-1"><IconPhoto size={16} />Captured</span> : "Required"}</span><input type="file" accept="image/*" capture="environment" className="hidden" onChange={(event) => { const file = event.target.files?.[0] ?? null; setEvidence(file); setEvidencePreview(""); if (!file) return; const reader = new FileReader(); reader.onload = () => setEvidencePreview(String(reader.result ?? "")); reader.readAsDataURL(file); }} /></label>{evidencePreview && <div className="mt-3 overflow-hidden rounded-xl border border-pca-border bg-black"><img src={evidencePreview} alt="Farm visit evidence preview" className="max-h-56 w-full object-contain sm:max-h-64" /></div>}<p className="mt-2 text-xs text-pca-muted">This opens the device camera on supported phones. Evidence is available to administrators only.</p></> : <label className="mt-4 block text-sm font-semibold">Reason for not visiting<textarea value={notVisitedReason} onChange={(e) => setNotVisitedReason(e.target.value)} className="mt-2 min-h-28 w-full rounded-lg border border-pca-border p-3 text-sm font-normal" placeholder="State why the visit could not be completed and the next action." /></label>}
