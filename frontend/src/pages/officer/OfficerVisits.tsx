...
export default function OfficerVisits() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {loggedOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4"><div className="max-h-[82vh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-pca-border p-5"><div><p className="text-xs font-bold uppercase tracking-wide text-pca-muted">Officer records</p><h2 className="mt-1 text-xl font-bold">Logged Visits</h2><p className="mt-1 text-sm text-pca-muted">Completed and missed visits are kept here after an outcome is recorded.</p></div><button type="button" onClick={() => setLoggedOpen(false)} className="rounded-lg border border-pca-border p-2"><IconX size={20} /></button></div><div className="max-h-[63vh] space-y-4 overflow-y-auto p-5">{loggedVisits.map((visit) => { const log = visitLogs.find((item) => item.visitId === visit.id); if (!log) return null; return <article key={visit.id} className="rounded-xl border border-pca-border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-pca-text">{visit.farm}</p><p className="mt-1 text-xs text-pca-muted">{visit.owner} · {formatVisitLine(visit.date, visit.slot)}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${log.visited ? "bg-pca-green-light text-pca-green" : "bg-pca-red-light text-pca-red"}`}>{log.visited ? "Visit completed" : "Visit not completed"}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-lg bg-pca-bg p-3"><p className="text-[11px] font-bold uppercase tracking-wide text-pca-muted">Visit purpose</p><p className="mt-1 text-sm text-pca-text">{visit.purpose}</p></div><div className="rounded-lg bg-pca-bg p-3"><p className="text-[11px] font-bold uppercase tracking-wide text-pca-muted">Recorded outcome</p><p className="mt-1 text-sm text-pca-text">{log.visited ? log.officerComment : log.notVisitedReason}</p></div></div>{log.farmerConfirmed !== null && <div className="mt-3 rounded-lg bg-pca-green-light p-3 text-sm text-pca-text"><strong>Farmer feedback{log.farmerRating ? ` · ${log.farmerRating}/5 stars` : ""}:</strong> {log.farmerComment || (log.farmerConfirmed ? "Visit confirmed without a written comment." : log.farmerReport || "The farmer reported that the visit was not completed.")}</div>}{log.adminFeedback && <div className="mt-3 rounded-lg bg-blue-50 p-3 text-sm text-blue-950"><strong>Administrator feedback{log.adminRating ? ` · ${log.adminRating}/5 stars` : ""}:</strong> {log.adminFeedback}</div>}</article>; })}{loggedVisits.length === 0 && <p className="py-8 text-center text-sm text-pca-muted">No logged visits yet.</p>}</div></div></div>}
