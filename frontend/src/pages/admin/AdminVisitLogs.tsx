...
export default function AdminVisitLogs() {
  ...
  </Card>{loggedOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4"><div className="max-h-[82vh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-pca-border p-5"><div><p className="text-xs font-bold uppercase tracking-wide text-pca-muted">Administrative audit</p><h2 className="mt-1 text-xl font-bold">Logged Visits of All Officers</h2><p className="mt-1 text-sm text-pca-muted">Review the scheduled visit, recorded outcome, and completed feedback trail.</p></div><button type="button" onClick={() => setLoggedOpen(false)} className="rounded-lg border border-pca-border p-2"><IconX size={20} /></button></div><div className="max-h-[64vh] overflow-y-auto p-5"><label className="mb-4 block text-sm font-semibold">Filter officer<select value={loggedOfficer} onChange={(event) => setLoggedOfficer(event.target.value)} className="mt-2 w-full rounded-lg border border-pca-border bg-white p-2.5 text-sm"><option value="all">All officers</option>{[...new Map(allLoggedVisits.map((log) => [log.officerId, log.officerName])).entries()].map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label><div className="space-y-4">{filteredLoggedVisits.map((log) => { const visit = scheduledVisits.find((item) => item.id === log.visitId); return <article key={log.id} className="rounded-xl border border-pca-border p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-semibold">{log.farm}</p><p className="mt-1 text-xs text-pca-muted">{log.officerName} · Recorded {log.recordedAt}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${log.visited ? "bg-pca-green-light text-pca-green" : "bg-pca-red-light text-pca-red"}`}>{log.visited ? "Visit completed" : "Visit not completed"}</span></div>{visit && <div className="mt-4 flex gap-3 rounded-lg bg-pca-bg p-3"><IconCalendarEvent size={18} className="mt-0.5 shrink-0 text-pca-green" /><div><p className="text-[11px] font-bold uppercase tracking-wide text-pca-muted">Scheduled visit</p><p className="mt-1 text-sm font-semibold text-pca-text">{formatVisitLine(visit.date, visit.slot)}</p><p className="mt-1 text-sm text-pca-muted">{visit.purpose}</p></div></div>}<div className="mt-3 rounded-lg bg-pca-bg p-3"><p className="text-[11px] font-bold uppercase tracking-wide text-pca-muted">Officer outcome</p><p className="mt-1 text-sm text-pca-text">{log.visited ? log.officerComment : log.notVisitedReason}</p></div>{log.farmerConfirmed !== null && <div className="mt-3 rounded-lg bg-pca-green-light p-3 text-sm text-pca-text"><strong>Farmer feedback{log.farmerRating ? ` · ${log.farmerRating}/5 stars` : ""}:</strong> {log.farmerComment || (log.farmerConfirmed ? "Visit confirmed without a written comment." : log.farmerReport || "Visit was reported as not completed.")}</div>}{log.adminFeedback && <div className="mt-3 rounded-lg bg-blue-50 p-3 text-sm text-blue-950"><strong>Administrator feedback{log.adminRating ? ` · ${log.adminRating}/5 stars` : ""}:</strong> {log.adminFeedback}</div>}</article>; })}{filteredLoggedVisits.length === 0 && <p className="py-8 text-center text-sm text-pca-muted">No logged visits match this officer.</p>}</div></div></div></div>}</div>;
}
