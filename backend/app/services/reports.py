...
def _report_footer(canvas, doc) -> None:
    ...
    canvas.drawString(inch * 0.65, 0.45 * inch, "CocoAnalytica - Coconut Leaf Condition Monitoring System")
...
def build_monthly_report_pdf(
    ...
    *,
    ...
    generated_label = generated_at or datetime.now(PHILIPPINE_TZ).strftime("%B %d, %Y, %I:%M %p PHT")
    meta = [
        ["Report Generated", generated_label, "Coverage Area", scope_label],
    ...
    story.append(_table(meta, [1.2 * inch, 2.25 * inch, 1.15 * inch, 2.3 * inch], header=False))
    ...
    story.append(_table(sector_rows, [0.95 * inch, 2.0 * inch, 0.85 * inch, 1.55 * inch, 1.4 * inch]))
...
def _finish_farmer_audit_report(
    ...
    *,
    ...
    story += [
        ...
        _table(rows, [0.9 * inch, 1.45 * inch, 1.25 * inch, 1.25 * inch, 0.8 * inch, 0.85 * inch]),
    ]
    doc.build(story, onFirstPage=_report_footer, onLaterPages=_report_footer)
    return buffer.getvalue()
    ...
    for row_index, row in enumerate(rows):
        ...
        for col_index, value in enumerate(row):
            ...
            if header and row_index == 0:
                style = header_style
            elif not header and col_index in (0, 2):
                style = label_style
            wrapped_row.append(_pdf_cell(value, style))
        wrapped_rows.append(wrapped_row)

    table = Table(wrapped_rows, colWidths=widths, repeatRows=1 if header else 0, splitByRow=True)
    style = [
